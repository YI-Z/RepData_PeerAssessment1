...
```
The mean of the total number of steps taken per day is `r mean`, and the median is `r med`.
...
```
Interval `r max_int` contains the maxium number of steps on average across all the days.
...
```
There are `r na_row` rows with missing values in the dataset.
...
```
The original data has been filled in with NA replaced by average value of the interval. The mean of the new total number of steps taken per day is `r mean_new`, and the median is `r med_new`. These value differ from mean and median values of estimations with NA ignored, which are `r mean` and `r med`, respectively.
